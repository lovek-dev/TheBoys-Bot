...
module.exports = {
    ...
    async execute(interaction, client) {
        ...
        const ticketCategory     = client.db.get(`summer_ticket_category_${interaction.guildId}`);
        const logsChannel        = client.db.get(`summer_logs_channel_${interaction.guildId}`);
        const transcriptChannel  = client.db.get(`summer_ticket_transcript_${interaction.guildId}`);
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: '📂 Ticket Category',       value: ticketCategory   ? `<#${ticketCategory}>`     : '❌ Not set — `/summerticket <category>`',    inline: true },
                { name: '📄 Transcript Channel',    value: transcriptChannel ? `<#${transcriptChannel}>` : '❌ Not set — `/tickettranscript #channel`',  inline: true },
                ...
                { name: '`/summerticket <category>`',    value: 'Set the category for ticket channels',         inline: true },
                { name: '`/summerticketpanel`',          value: 'Send the ticket panel in this channel',        inline: true },
                { name: '`/tickettranscript #channel`',  value: 'Set where ticket transcripts are saved',       inline: true },
